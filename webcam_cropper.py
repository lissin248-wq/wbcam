...
def detect_faces_opencv(image_np: np.ndarray):
    """
    Detecta faces na imagem usando Haar Cascade do OpenCV (sem deps extras).
    Retorna lista de (x, y, w, h) das faces encontradas.
    """
...
def box_contains_face(box, faces, min_overlap_ratio=0.3):
    """
    Verifica se uma box contém (ou sobrepõe significativamente) alguma face detectada.
    Retorna True se alguma face tiver pelo menos min_overlap_ratio de área dentro da box.
    """
    bx1, by1, bx2, by2 = box
    for (fx, fy, fw, fh) in faces:
        ...
        face_area = fw * fh
        if inter_area / face_area >= min_overlap_ratio:
            return True
    return False
...
def filter_and_select_best_box(
    ...
    Usa detecção de face como critério primário de seleção:
      - Boxes que contêm um rosto humano são fortemente preferidas.
      - Entre as que têm face (ou entre as sem face, se nenhuma tiver),
        desempata pelo score do DINO.
    """
    img_w, img_h = image_size
    img_area = img_w * img_h

    # Detecta faces na imagem inteira (uma vez só)
    faces = detect_faces_opencv(image_np) if image_np is not None else []
    if len(faces) > 0:
        print(f"  [FACE] {len(faces)} face(s) detectada(s) na imagem")
    else:
        print("  [FACE] Nenhuma face detectada — usando só score do DINO")
    ...
    # Separa boxes que contêm face das que não contêm
    if len(faces) > 0:
        face_indices = [i for i in valid_indices if box_contains_face(boxes[i], faces)]
        no_face_indices = [i for i in valid_indices if i not in face_indices]

        if face_indices:
            print(f"  [FACE] {len(face_indices)} box(es) com face detectada — priorizando")
            candidate_indices = face_indices
        else:
            print("  [FACE] Nenhuma box válida contém face — usando score do DINO")
            candidate_indices = no_face_indices if no_face_indices else valid_indices
    else:
        candidate_indices = valid_indices

    # Seleciona a de maior score entre as candidatas
    candidate_scores = scores[candidate_indices]
    best_idx = candidate_indices[np.argmax(candidate_scores)]
...
def crop_webcam(
    ...
    best_box, best_score, best_label = filter_and_select_best_box(
        boxes, scores, labels, (img_w, img_h), image_np=image_np
    )
